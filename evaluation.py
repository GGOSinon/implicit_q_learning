...
def take_video(seed: int, agent: nn.Module, render_env: gym.Env, termination_fn) -> Dict[str, float]:
    ...
    while True:
        ...
        q_value = agent.critic(observation[None], action[None])
...
def evaluate(seed: int, agent: nn.Module, envs: gym.vector.VectorEnv, video_path: str, step:int, model_observe=None) -> Dict[str, float]:
    stats = {'return': [], 'length': []} 
    ...
    if isinstance(envs, list):
        ...
        if model_observe is not None:
            _states = {'stoch': np.zeros((num_epsiodes, 32, 32)), 'deter': np.zeros((num_episodes, 200))}
        dones = np.array(dones, dtype=bool)
        for j in range(10000):
            if np.all(dones): break
            if model_observe is not None:
                embed = model_observe.encoder(observations)
                if j == 0: _states = model_observe.apply_fn.obs_step(key, 0., 0., embed, 1.0) 
                else: _states = model_observe.apply_fn.obs_step(key, _states, _actions, embed, 0.0) 
                _actions = agent.sample_actions(_states, temperature=0.0)
            else:
    ...
    '''
    q_values = agent.critic(states, actions)
