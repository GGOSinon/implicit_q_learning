...
def ms_update_actor(key: PRNGKey, actor: Model, critic: Model, value: Model, model: Model,
        batch: Batch,
        discount: float, temperature: float, sac_alpha: float, lamb: float, H: int, beta: float) -> Tuple[Model, InfoDict]:

    N, T = batch.observations.shape[:2]
    def actor_loss_fn(actor_params: Params) -> Tuple[jnp.ndarray, InfoDict]:
        Robs = batch.observations.reshape(N * T, -1)
        dist = actor.apply({'params': actor_params}, Robs, temperature, training=True, rngs={'dropout': key})
        Ra = dist.sample(seed=key) 
        states, rewards, actions, masks, weights = [Robs], [], [Ra], [], [jnp.ones(N*T)]
        keys = [key]
        ...
        q_rollout = q_rollout[0] # [N, H] 
        log_probs = jnp.stack(log_probs, axis=1) # [N, H]
        policy_std = jnp.stack(policy_std, axis=1)
        weights = jnp.stack(weights, axis=1)

        #q_rollout = q_rollout.reshape(N, num_repeat)
        #q_rollout_mean = q_rollout.mean(axis=1)
        #q_rollout_std = jnp.sqrt(jnp.mean((q_rollout - q_rollout_mean[:, None]) ** 2, axis=1) + 1e-8)
        #q_rollout = q_rollout_mean - beta * q_rollout_std
        actor_loss_rollout = -q_rollout.mean() + sac_alpha * log_probs.mean()

        ############################## Data rollout ################################
        q_data = []
        value_estimate = value(batch.next_observations[:, -1])
        for i in reversed(range(T)):
            next_value = value(batch.next_observations[:, i])
            value_estimate = batch.rewards[:, i] + discount * batch.masks[:, i] * (lamb * value_estimate + (1-lamb) * next_value)
            #q_rollout.append(lamb * value_estimate + (1 - lamb) * q_values[i])
            q_data.append(value_estimate)
        q_data = list(reversed(q_data)) # [N, T]

        advs_data, log_probs_data = [], []
        for i in range(T):
            Robs = batch.observations[None, :, i].repeat(10, axis=0)
            Ra = actor(Robs).sample(seed=keys[-1]); rng, key0 = jax.random.split(keys[-1], 2); keys.append(key0)
            q1, q2 = critic(Robs, Ra); v = jnp.minimum(q1, q2).mean(axis=0)
            advs = (q_data[i] - v); advs_data.append(advs)

            dist = actor(batch.observations[:, i])
            log_prob = dist.log_prob(batch.actions[:, i]); log_probs_data.append(log_prob)
        
        #advs_data = jnp.stack(advs_data, axis=1)
        log_probs_data = jnp.sum(jnp.stack(log_probs_data, axis=1), axis=1)
        advs_data = advs_data[0]
        #log_probs_data = log_probs_data[0]
        exp_advs_data = jnp.minimum(jnp.exp(advs_data * 10.0), 100.0)

        actor_loss_data = -sg(exp_advs_data) * log_probs_data
        actor_loss_data = actor_loss_data.mean()

        #jax.debug.print('{x}, {y}, {z}, {w}, {u}', x=actor_loss, y=log_probs.mean(), z=q_rollout.mean(), w=q_rollout_std.mean(), u=num_repeat)
        #print(log_probs.shape, weights.shape, policy_std.shape, loss_weights.shape)

        actor_loss = actor_loss_data
        return actor_loss, {'actor_loss': actor_loss, 'q_rollout': q_rollout.mean(), 'advs_data': advs_data.mean(),
