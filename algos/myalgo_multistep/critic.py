...
def update_v(key: PRNGKey, critic: Model, value: Model, actor: Model, data_batch: Batch, model_batch: Batch,
        expectile: float, model_batch_ratio: float) -> Tuple[Model, InfoDict]:

    num_repeat = 10
    Robs = data_batch.observations[None].repeat(num_repeat, axis=0)
    Ra = actor(Robs).sample(seed=key)
    q1_rollout, q2_rollout = critic(Robs, Ra)
    q_model = jnp.minimum(q1_rollout, q2_rollout)
    q_model = jnp.mean(q_model, axis=0)

    q1_data, q2_data = critic(data_batch.observations, data_batch.actions)
    q_data = jnp.minimum(q1_data, q2_data)

    #actions = actor(data_batch.observations, 0.0).sample(seed=key)
    #q1_data_pi, q2_data_pi = critic(data_batch.observations, actions)
    #q_data_pi = jnp.minimum(q1_data_pi, q2_data_pi)

    #target_q = jnp.maximum(q_model, q_data)
    target_q = q_data
    clip_ratio = (q_model < q_data).mean()

    def value_loss_fn(value_params: Params) -> Tuple[jnp.ndarray, InfoDict]:
        #v_model = value.apply({'params': value_params}, model_batch.observations)
        #value_loss_model = loss(q_model, v_model, 0.5).mean()

        v_data = value.apply({'params': value_params}, data_batch.observations)
        #value_loss_data = loss(q_data, v_data, 0.9).mean()

        #value_loss = jnp.concatenate([value_loss_model, value_loss_data], axis=0).mean()
        #value_loss = (1 - model_batch_ratio) * value_loss_data + model_batch_ratio * value_loss_model
        value_loss = loss(target_q, v_data, 0.9).mean()

        return value_loss, {
            'value_loss': value_loss,
            #'v_model': v_model.mean(),
            'v_data': v_data.mean(),
            'argmax_adv': (q_data - q_model).mean(),
...
# COMBO
def update_q(key: PRNGKey, critic: Model, value: Model, actor: Model, model: Model,
    ...
    N, T = model_batch.observations.shape[:2]

    ################ a rollout ###############
    Robs = model_batch.observations.reshape(N*T, -1)
    ...
    def critic_loss_fn(critic_params: Params) -> Tuple[jnp.ndarray, InfoDict]:
        ...
        critic_info = {
            ...
            'expectile': expectile, 'lambda': lamb.mean(), 'lambda_min': lamb.min(), 'lambda_max': lamb.max(),
            'state_max': jnp.abs((states - value.apply_fn.scaler[0]) / value.apply_fn.scaler[1] * loss_weights[:, None]).max(), #'critic_reg_loss': critic_reg_loss,
        }
